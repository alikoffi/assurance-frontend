...
@Component({
	...
	template: `
		<button (click)="toggleMenu()" pTooltip="Profil" [tooltipDisabled]="isTooltipDisabled()">
			<em class="fa fa-user-circle-o" style="font-size: 20px;"></em>
			<span class="text-start">
                <strong>{{ utilisateurConnecte?.nomEtPrenomAbrege() }}</strong>
                <small>{{ utilisateurConnecte?.role }}</small>
            </span>
		...
		<ul *ngIf="menuProfileActive()" [@menu]="isHorizontal() ? 'overlay' : 'inline'">
			<li pTooltip="Mot de passe" [tooltipDisabled]="isTooltipDisabled()">
				<button [routerLink]="['/mot-de-passe']">
